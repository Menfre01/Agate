...
|------|------|------|
| 路由入口聚合 | `src/index.ts` | ✅ |
| 数据库迁移 | `src/db/schema.sql`, `npm run db:migrate` | ✅ |
| 部署配置 | `wrangler.prod.jsonc`, `docs/DEPLOYMENT.md` | ✅ |
| 生产部署 | `npm run deploy`, `npm run deploy:prod` | ✅ |
| 冒烟测试 | `scripts/smoke-test.sh` | ✅ |

**扩展内容**:
- 种子数据脚本 (`scripts/seed-data.sql`)
- 部署文档 (`docs/DEPLOYMENT.md`)
- 生产环境配置模板 (`wrangler.prod.jsonc`)
- E2E 测试套件 (`tests/e2e/`)

**E2E 测试功能**:
- 支持自定义供应商和模型配置
- 自动创建测试数据（公司、部门、用户、供应商、模型）
- 完整的 API 测试流程（认证、创建、请求、统计）
- 自动清理测试数据，实现测试闭环
